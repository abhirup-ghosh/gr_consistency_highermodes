...
\newcommand{\ajitm}[1]{\textcolor{blue}{Ajitm: #1}}
\newcommand{\io}{\iota}
\newcommand{\p}{\phi}
\newcommand{\vp}{\varphi}
...
Indeed, this test requires the higher order modes of the radiation to be detected with sufficient signal-to-noise ratio (SNR). This entail the observation of binaries with large mass ratios and/or highly misaligned spins with high inclination angles (angle between the orbital angular momentum and the line of sight). Since the gravitational radiation is primarily beamed parallel/anti-parallel to the orbital angular momentum, GW observations have a selection bias towards binaries with small inclination angles, and hence the contribution from higher modes is likely to be small for most observed systems. However, considering Advanced LIGO and Virgo are expected to detect hundreds of binary black hole mergers in the next few years, we are likely to detect a small number of high-mass ratio binaries in inclined orbits which enables this test to be performed~\cite{Dhanpal:2018ufk}. There is already preliminary evidence of higher modes in one of the binary black hole events detected by LIGO and Virgo during their second observing run~\cite{Chatziioannou:2019dsz}. 

%Most of the tests of GR performed using past GW observations rely on the power in the leading quadrupolar modes \cite{LSC_2016grtests,gw170104,ligo2019tests,abbott2019tests,abbott2017gravitational}. However there have been work~
% AJITH: If we want to discuss the importance of higher modes in GW data analysis (which, in my opinion, is a tangential point), I am not sure if these are the papers to cite. For eg. 
% Bustillo:2015ova,2015arXiv150100918C - this is on the construction of multi mode NRPN hybrids 
% Varma:2014hm - this is on the systematic errors due to neglecting higher modes 
% CalderonBustillo:2016hm,Bustillo2016_hm - this is on the loss of detection efficiency due to neglecting precession and higher modes 
% \cite{Bustillo:2015ova,2015arXiv150100918C,Varma:2014hm,CalderonBustillo:2016hm,Bustillo2016_hm}
%  that highlight the importance of including higher harmonics in our analyses. In fact, a reanalysis of the GW signals detected during the first two observing runs of Advanced LIGO and Virgo, do find evidence of the presence of higher modes~\cite{Chatziioannou:2019dsz}. 
...
\ajitm{Figure~\ref{fig:posterior_low_spin} and Figure~\ref{fig:posterior_high_spin} show the posteriors in the deviation parameters $\Delta M_c/M_c$ and $\Delta q$ introduced in Eq.\eqref{eq:test_1} estimated from simulated binary black hole events with different values of spin. The consistency test is always performed using the non-spinning phenomenological waveform family~\cite{Mehta:2017jpq} as the base GR waveform. The dimensionless spins of the component black holes are shown in the legends. Figure~\ref{fig:posterior_low_spin} shows the posteriors in the deviation parameters $\Delta M_c/M_c$ and $\Delta q$ for low spins, while Figure~\ref{fig:posterior_high_spin} shows the same for high spins.   It can be seen that, posteriors estimated from spinning simulated waveforms are consistent with the GR values ($\Delta M_c = \Delta q = 0)$. However, the widths of the deviation parameters $\Delta M_c/M_c$ and $\Delta q$ increase significantly for high spins of the primary component black hole. This suggests that one should use an accurate spinning waveform model if one wants to perform such tests on highly spinning signals}. \tousif{I guess we can also mention 'For high spin injections, though the posteriors of $\Delta M_c/M_c$ and $\Delta q$ broadly are consistent with GR value \{0,0\} at the 90\% level, the peaks of the posteriors shows a bias from the injection'.}
%The effective spin $\chi_\mathrm{eff} = m_1 \chi_1 + m_2 \chi_2)/(m_1+m_2)$ of the simulated (target) waveforms is shown in the legends.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htb] 
\begin{center}
\includegraphics[width=3.4in]{figs/M_80_q_4_sur_phenom_diffchi.pdf}
\end{center} 
\caption{Posteriors of the deviation parameters $\Delta M_c/M_c$ and $\Delta q$  for the spinning injections along with the non-spinning injection (magenta dashed curve). The legends show the spins of the component black holes. We see that  width of the posteriors are larger for non-zero spins compared to the non-spinning binary black holes. }
\label{fig:posterior_low_spin}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[htb] 
	\begin{center}
		\includegraphics[width=3.4in]{figs/M_80_q_4_sur_phenom_diffchi_high_chi_eff.pdf}
	\end{center} 
\caption{Same as Figure~\ref{fig:posterior_low_spin} but for high primary component spins. We can see that the width of the deviation parameters increase significantly. \ajitm{I will put a better picture latter.}}
\label{fig:posterior_high_spin}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
