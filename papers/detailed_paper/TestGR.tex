...
\newcommand{\tousif}[1]{\textcolor{blue}{Tousif: #1}}
\newcommand{\io}{\iota}
\newcommand{\p}{\phi}
\newcommand{\vp}{\varphi}
...
\ajith{What exactly are the parameters that are sampled? $t_0$ and $\theta$, $\phi$?}
 \tousif{$t_0$ and $\theta$, $\phi$, $\psi$ where $t_0$ is the time of arrival of the signal at the center of the earth. From there, $\{ t_{0,H}, t_{0,L}, t_{0,V}\}$ was computed.}
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.50]{figs/2d_c3_c4_M_80_q_9_SNR_50.pdf}
	\end{center} 
	\caption{The figure shows posterior probability distribution of the deviation parameters $c_2$, $c_3$ and $c_4$ for binaries with total mass $M=80$ $M_{\odot}$ and mass ratio $q=1/9$. The SNR of the signal is 50.}
	\label{fig:constraint_c2_c3_c4}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\tousif{One may also  allow all of the deviation parameters i.e. $c_{21}$, $c_{33}$ and $c_{44}$ to vary simultaneously and estimate their posterior. We show one example of the estimated posteriors of $c_{21}$, $c_{33}$ and $c_{44}$ in Fig \ref{fig:bounds_c2c34c3c4} for a binary with total mass $M=80$ $M_{\odot}$ and mass ratio $q=1/9$. The SNR of the signal is 50.}
