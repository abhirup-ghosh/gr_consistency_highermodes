...
model = 'gr'
model = 'mod_gr'
data=np.genfromtxt('data_%s.txt'% model, delimiter=' ')
...
plt.plot(0, 0, 'k+', ms=12, mew=2) # for mod gr make the marker k+ , for gr make the marker w+
...
plt.tight_layout()
plt.savefig('triangle_plot_%s.png' %model, dpi=300)
plt.close()
