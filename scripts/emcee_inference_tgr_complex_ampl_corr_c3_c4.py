...
def lnlike(param_vec, data1, data2, data3, freq1, psd, psdv, f_low, f_cut):
        ...
        Mc, q, absc3, phi3, absc4, phi4, dL, i, t0, phi0,  ra, sin_dec, pol= param_vec
        c3=absc3*np.exp(1j*phi3)
        c4=absc4*np.exp(1j*phi4)
	...
	# three detectors
        ...
        f1, hpf1, hcf1 = phhsi.phenomhh_waveform_cmplx_ampcorr_SI(Mc, q, 0.0, c3, c4, dL, i, t01, (phi0 %(2.*pi)), f_low, df, Ncs)
        f2, hpf2, hcf2 = phhsi.phenomhh_waveform_cmplx_ampcorr_SI(Mc, q, 0.0, c3, c4, dL, i, t02, (phi0 %(2.*pi)), f_low, df, Ncs)
        f3, hpf3, hcf3 = phhsi.phenomhh_waveform_cmplx_ampcorr_SI(Mc, q, 0.0, c3, c4, dL, i, t03, (phi0 %(2.*pi)), f_low, df, Ncs)
...
def lnprior(param_vec):
        Mc, q, absc3, phi3, absc4, phi4, dL, i, t0, phi_0, ra, sin_dec, pol = param_vec
        if 1 < Mc < 200 and 0.05 < q <= 1. and  0.0 < absc3 <= 10.0 and -pi <= phi3 <= pi  and  0.0 < absc4 <= 10.0 and -pi <= phi4 <= pi and 1.<dL<10000 and 0.<= i <= pi and 0.<= t0 <= 15. and -pi <= phi_0 <= 3.*pi and 0. <= ra < 2.*pi and -1. <= sin_dec <= 1. and 0. <= pol <= pi:
...
mc_init, q_init, absc3_init, phi3_init, absc4_init, phi4_init, dL_init, iota_init, t0_init, phi0_init, ra_init, sin_dec_init, pol_init = result
...
# save the data
np.savetxt(out_dir+'/emcee_samples.dat', samples, header='mc q abs_c3 arg_c3 abs_c4 arg_c4 dL i t0 phi0 ra sin(dec) pol')
...
plt.ylabel('q')
plt.subplot(723)
plt.plot(absc3_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(absc3_init + np.std(absc3_chain, axis=1), 'r')
plt.axhline(y=absc3_init, color='g')
plt.ylabel('|c3|')
plt.subplot(724)
plt.plot(phi3_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(phi3_init + np.std(phi3_chain, axis=1), 'r')
plt.axhline(y=phi3_init, color='g')
plt.ylabel('arg(c3)')
plt.subplot(725)
plt.plot(absc4_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(absc4_init + np.std(absc4_chain, axis=1), 'r')
plt.axhline(y=absc4_init, color='g')
plt.ylabel('|c4|')
plt.subplot(726)
plt.plot(phi4_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(phi4_init + np.std(phi4_chain, axis=1), 'r')
plt.axhline(y=phi4_init, color='g')
plt.ylabel('arg(c4)')
plt.subplot(727)
...
# corner plots
plt.figure()
corner.corner(samples, labels=['mc', 'q', '|c3|', 'arg(c3)', '|c4|','arg(c4)','dL', 'i', 't0', 'phi0', 'ra', 'sin(dec)', 'pol'])
