...
def lnlike(param_vec, data1, data2, data3, freq1, psd, f_low, f_cut):
        ...
        Mc, q, absc, phi, dL, i, t0, phi0, ra, sin_dec, pol= param_vec
        dec=np.arcsin(sin_dec)
	c1=absc*np.exp(1j*phi)
        # three detectors
        ...
        f1, hpf1, hcf1 = phhsi.phenomhh_waveform_cmplx_ampcorr_SI(Mc, q, c1, c1, c1, dL, i, t01, (phi0 %(2.*pi)), f_low, df, Ncs)
        f2, hpf2, hcf2 = phhsi.phenomhh_waveform_cmplx_ampcorr_SI(Mc, q, c1, c1, c1, dL, i, t02, (phi0 %(2.*pi)), f_low, df, Ncs)
        f3, hpf3, hcf3 = phhsi.phenomhh_waveform_cmplx_ampcorr_SI(Mc, q, c1, c1, c1, dL, i, t03, (phi0 %(2.*pi)), f_low, df, Ncs)
...
def lnprior(param_vec):
	Mc, q, absc, phi, dL, i, t0, phi_0, ra, sin_dec, pol = param_vec
	if 1 < Mc < 200 and 0.05 < q <= 1. and  0.0 < absc <= 10.0 and -pi <= phi <= pi   and 1.<dL<10000 and 0.<= i <= pi and 0.<= t0 <= 15. and -pi <= phi_0 <= 3.*pi and 0. <= ra < 2.*pi and -1. <= sin_dec <= 1. and 0. <= pol <= pi:
...
mc_init, q_init, absc_init, phi_init, dL_init, iota_init, t0_init, phi0_init, ra_init, sin_dec_init, pol_init = result
...
# save the data
np.savetxt(out_dir+'/emcee_samples.dat', samples, header='mc q abs_c1 arg_c1 dL i t0 phi0 ra sin(dec) pol')
...
plt.ylabel('q')
plt.subplot(623)  
plt.plot(absc_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(absc_init + np.std(absc_chain, axis=1), 'r')
plt.axhline(y=absc_init, color='g')
plt.ylabel('|c1|')
plt.subplot(624)
plt.plot(phi_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(phi_init + np.std(phi_chain, axis=1), 'r')
plt.axhline(y=phi_init, color='g')
plt.ylabel('arg(c1)')
plt.subplot(625)
...
# corner plots
plt.figure()
corner.corner(samples, labels=['mc', 'q', '|c1|', 'arg(c1)', 'dL', 'i', 't0', 'phi0', 'ra', 'sin(dec)', 'pol'])
