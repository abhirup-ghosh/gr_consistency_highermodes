'''
PE code for the cs gravity inspired polarization tests
'''
import os, sys
...
def lnlike(param_vec, data1, data2, data3, freq1, psd, f_low, f_cut):
	...
	Mc, q, v, dL, i, t0, phi0, ra, sin_dec, pol= param_vec
	...
	f1, hpf1, hcf1 = phhsi.phenomhh_waveform_pol_corr(Mc, q, dL, i, t01, (phi0 %(2.*pi)), f_low, df, Ncs)
	f2, hpf2, hcf2 = phhsi.phenomhh_waveform_pol_corr(Mc, q, dL, i, t02, (phi0 %(2.*pi)), f_low, df, Ncs)
	f3, hpf3, hcf3 = phhsi.phenomhh_waveform_pol_corr(Mc, q, dL, i, t03, (phi0 %(2.*pi)), f_low, df, Ncs)
	...
	signal1=Fp1*(hpf1+ 1j*v*f1*hcf1) + Fc1*(hcf1-1j*v*f1*hpf1)
	signal2=Fp2*(hpf2+ 1j*v*f2*hcf2) + Fc2*(hcf2-1j*v*f2*hpf2)
	signal3=Fp3*(hpf3+ 1j*v*f3*hcf3) + Fc3*(hcf3-1j*v*f3*hpf3)
...
def lnprior(param_vec):
	Mc, q, v, dL, i, t0, phi_0, ra, sin_dec, pol = param_vec
	if 1 < Mc < 200 and 0.05 < q <= 1. and  -5.0 < v <= 5.0 and 1.<dL<10000 and 0.<= i <= pi and 0.<= t0 <= 15. and 0.0 <= phi_0 <= 2.*pi and 0. <= ra < 2.*pi and -1. <= sin_dec <= 1. and 0.0 <= pol <= pi:
...
mc_init, q_init, v_init, dL_init, iota_init, t0_init,  phi0_init, ra_init, sin_dec_init, pol_init = result
...
mc_chain, q_chain, v_chain,  dL_chain, iota_chain, t0_chain, phi0_chain, ra_chain, sin_dec_chain, pol_chain = sampler.chain[:, :, 0].T, sampler.chain[:, :, 1].T, sampler.chain[:, :, 2].T, sampler.chain[:, :, 3].T, sampler.chain[:, :, 4].T, sampler.chain[:, :, 5].T, sampler.chain[:, :, 6].T, sampler.chain[:, :, 7].T, sampler.chain[:, :, 8].T, sampler.chain[:, :, 9].T
...
# save the data
np.savetxt(out_dir+'/emcee_samples.dat', samples, header='mc q v dL i t0 phi0 ra sin(dec) pol')
...
plt.ylabel('q')
plt.subplot(723)
plt.plot(v_chain, color="k", alpha=0.4, lw=0.5)
plt.plot(v_init + np.std(v_chain, axis=1), 'r')
plt.axhline(y=v_init, color='g')
plt.ylabel('v')
plt.subplot(724)
...
# corner plots
plt.figure()
corner.corner(samples, labels=['mc', 'q', 'v', 'dL', 'i', 't0','phi0', 'ra', 'sin(dec)', 'pol'])
