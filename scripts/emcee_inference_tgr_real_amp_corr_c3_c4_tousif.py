...
def lnlike(param_vec, data1, data2, data3, freq1, psd, psdv, f_low, f_cut):
        ...
        Mc, q, c3, c4,  dL, i, t0, phi0,  ra, sin_dec, pol= param_vec
	...
	# three detectors
        ...
        f1, hpf1, hcf1 = phhsi.phenomhh_waveform_real_ampcorr_SI(Mc, q, 0.0, c3, c4, dL, i, t01, (phi0 %(2.*pi)), f_low, df, Ncs)
        f2, hpf2, hcf2 = phhsi.phenomhh_waveform_real_ampcorr_SI(Mc, q, 0.0, c3, c4, dL, i, t02, (phi0 %(2.*pi)), f_low, df, Ncs)
        f3, hpf3, hcf3 = phhsi.phenomhh_waveform_real_ampcorr_SI(Mc, q, 0.0, c3, c4, dL, i, t03, (phi0 %(2.*pi)), f_low, df, Ncs)
...
def lnprior(param_vec):
        Mc, q, c3, c4, dL, i, t0, phi_0, ra, sin_dec, pol = param_vec
        if 1 < Mc < 200 and 0.05 < q <= 1. and  -10.0 < c3 <= 10.0  and  -10.0 < c4 <= 10.0 and 1.<dL<10000 and 0.<= i <= pi and 0.<= t0 <= 15. and -pi <= phi_0 <= 3.*pi and 0. <= ra < 2.*pi and -1. <= sin_dec <= 1. and 0. <= pol <= pi:
...
mc_init, q_init, absc3_init, absc4_init, dL_init, iota_init, t0_init, phi0_init, ra_init, sin_dec_init, pol_init = result
...
# save the data
np.savetxt(out_dir+'/emcee_samples.dat', samples, header='mc q c3 c4 dL i t0 phi0 ra sin(dec) pol')
...
# corner plots
plt.figure()
corner.corner(samples, labels=['mc', 'q', 'c3', 'c4','dL', 'i', 't0', 'phi0', 'ra', 'sin(dec)', 'pol'])
