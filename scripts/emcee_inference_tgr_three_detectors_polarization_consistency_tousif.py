...
def lnlike(param_vec, data1, data2, data3, freq1, psd, psdv, f_low, f_cut):
	...
	# generate the waveforms [time shifted] 
        f1, hpf1, hcf1 = phhsi.phenomhh_waveform_modpol_SI(Mc, q, Mc1, q1, dL, i, t01, (phi0 %(2.*pi)), f_low, df, Ncs)
        f2, hpf2, hcf2 = phhsi.phenomhh_waveform_modpol_SI(Mc, q, Mc1, q1, dL, i, t02, (phi0 %(2.*pi)), f_low, df, Ncs)
        f3, hpf3, hcf3 = phhsi.phenomhh_waveform_modpol_SI(Mc, q, Mc1, q1, dL, i, t03, (phi0 %(2.*pi)), f_low, df, Ncs)
...
def lnprior(param_vec):
	...
	if 1 < Mc < 200 and 0.05 < q <= 1. and  1 < Mc1 < 200 and 0.05 < q1 <= 1. and 1.<dL<10000 and 0.<= i <= pi and 0.<= t0 <= 15. and 0.0 <= phi_0 <= 2.*pi and 0. <= ra <= 2.*pi and -1. <= sin_dec <= 1. and 0.0 <= pol <= pi:
...
# save the data
np.savetxt(out_dir+'/emcee_samples.dat', samples, header='mc q mc1 q1 dL i t0 phi0 ra sin_dec pol')
