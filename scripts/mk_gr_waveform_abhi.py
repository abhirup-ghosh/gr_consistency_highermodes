...
out_file = 'GR_M_%.2f_Mc_%.2f_q_%.2f_snr_25_i_%.2f_dL_%.2f_phiref_%.2f_t0_%.2f_ra_%.2f_dec_%.2f_psi_%.2f_flow_20Hz.dat'%(M,Mc,q,iota, r, Psi_ref, t0, ra, dec, pol)
...
np.savetxt('../injections/'+out_file, np.c_[f[:-1],datar[:-1],datai[:-1],psd[:-1]])

print "python emcee_inference_tgr_abhi.py -d ../injections/%s -o ../runs/M_%d_q_%d_iota_%d -i '%.2f, %.2f, %.2f, %.2f, %.2f, %.2f, 6., 3.14'"%(out_file,M,1./q, np.ceil(np.degrees(iota)), Mc, q, Mc, q, r, iota)
