...
out_dir = '/home/tousif.islam/gr_consistency_highermodes_old/gwsur_phenom/fft_corrected/injection_15thjuly'
...
M=40.          ###### Total Mass in M_SUN
q=1./4.         ###### Mass ratio
SNR_req=25.    ###### Required SNR
iota=pi/3.0
Psi_ref=-0.87#np.pi

t0=0.0#1126285216.          ###### time of arrival at the earth center
t_gps=t0
...
'''
out_file = 'GR_M_%.2f_q_%.2f_snr_%.1f_i_%.2f_dL_%.2f_phiref_%.2f_t0_%.2f_ra_%.2f_dec_%.2f_psi_%.2f_flow_20Hz'%(M,q,SNR_tot,iota, r1, Psi_ref, t0, ra, dec, pol)
...
plt.legend()
#plt.savefig(out_dir + '/M_%.2f_q_%.2f_snr_%.2f_iota_%.2f_phi0_%.2f_signal_three_detectors.png'%(M,q,SNR_tot,iota,Psi_ref), dpi=200)
...
#np.savetxt(out_dir + '/%s_initial.dat'%out_file, np.c_[Mc, q, Mc, q, r1, iota, t0,  Psi_ref, ra, np.sin(dec), pol], header='Mc, q, Mc, q, dL, iota, t0, Psi_ref, ra, dec, pol')
