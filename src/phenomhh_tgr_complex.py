...
def hpfhcf_phen_pieces(f, c2, c3, c4, l, m, eta, t0, phi0, chi1, chi2, ampO, phaseO, iota, phi, qCase, Psi_ref):
  ...
  if l==2 and m==1:
	hpf = ((1+np.conj(c2))*dm2lmM*(-1)**l + (1+c2)*dm2lmP)*hlmfp_phen*np.exp(-1j*m*Psi_ref)
        hcf = (-1j)*((1+np.conj(c2))*dm2lmM*(-1)**l - (1+c2)*dm2lmP)*hlmfp_phen*np.exp(-1j*m*Psi_ref)
  if l==3 and m==3:
	hpf = ((1+np.conj(c3))*dm2lmM*(-1)**l + (1+c3)*dm2lmP)*hlmfp_phen*np.exp(-1j*m*Psi_ref)
        hcf = (-1j)*((1+np.conj(c3))*dm2lmM*(-1)**l - (1+c3)*dm2lmP)*hlmfp_phen*np.exp(-1j*m*Psi_ref)
  elif l==4 and m==4:
	hpf = ((1+np.conj(c4))*dm2lmM*(-1)**l + (1+c4)*dm2lmP)*hlmfp_phen*np.exp(-1j*m*Psi_ref)
        hcf = (-1j)*((1+np.conj(c4))*dm2lmM*(-1)**l - (1+c4)*dm2lmP)*hlmfp_phen*np.exp(-1j*m*Psi_ref)
  return hpf, hcf
